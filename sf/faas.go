...
func NewFaaSDesc(fnName string, f FaaS) (*FaaSDesc, error) {
	...
	// 确保  FaaS func(context.Context, Flow, ...interface{}) error 形参列表，存在context.Context 和 sf.Flow

	// 是否包含sf.Flow类型的形参
	containsSfFlow := false
	...
	if !containsSfFlow {
		// 不包含sf.Flow类型的形参，返回错误
		return nil, errors.New("function parameters must have sf.Flow param, please use FaaS type like: [type FaaS func(context.Context, Flow, ...interface{}) error]")
	}
...
// isFlowType 判断传递进来的 paramType 是否是 sf.Flow 类型
