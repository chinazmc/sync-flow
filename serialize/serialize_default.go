...
func (f *DefaultSerialize) UnMarshal(arr common.SfRowArr, r reflect.Type) (reflect.Value, error) {
	// 确保传入的类型是一个切片
...
// Marshal 用于将指定类型的值序列化为 SfRowArr(json 序列化)。
func (f *DefaultSerialize) Marshal(i interface{}) (common.SfRowArr, error) {
	var arr common.SfRowArr
...
// 尝试断言为结构体或指针
func unMarshalStruct(row common.SfRow, elemType reflect.Type) (reflect.Value, error) {
...
func unMarshalJsonString(row common.SfRow, elemType reflect.Type) (reflect.Value, error) {
	// 判断源数据是否可以断言成string
	str, ok := row.(string)
	if !ok {
...
func unMarshalJsonStruct(row common.SfRow, elemType reflect.Type) (reflect.Value, error) {
	// 将 row 序列化为 JSON 字符串
